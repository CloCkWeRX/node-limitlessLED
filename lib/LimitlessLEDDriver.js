 ...
 */
LimitlessLEDDriver.prototype.saveToCloud = function() {
  var self = this;
...
LimitlessLEDDriver.prototype.getExistingHubOptions = function() {
  var hubOptions = [];
  for ( var hubId in this._opts.lightHubs ) {
    var hubConfig = this._opts.lightHubs[hubId];
...
LimitlessLEDDriver.prototype.config = function(rpc, pushScreen) {
  ...
  if (!rpc) {
    ...
    hubSelectField.options = this.getExistingHubOptions( );
  ...
  switch (rpc.method) {
    ...
    case 'manual_remove_hub':
      var hubId = params.lll_hub;
      
      if ( typeof this.lightHubObjects[hubId] != 'undefined' ) {
        this.lightHubObjects[hubId].deregister( );
      }
      
      delete this.lightHubObjects[hubId];
      delete this._opts.lightHubs[hubId];
      this.saveToCloud( );
    ...
    case 'manual_group_add':
      var result = configMessages.manual_group_add;
    ...
    case 'manual_group_add_post':
      ...
      this.lightHubObjects[hubId].addLightGroup(params.lll_group, params.lll_colour);
