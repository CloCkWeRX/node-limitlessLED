...
function LimitlessLightGroup(hub, lightGroup) {
  ...
  this.G = lightGroup.number + lightGroup.colorType + hub.getDeviceSuffix(); // G is a string a represents the channel
  ...
  if ( data[0] == '{' ) {
    ...
    if (data == BLACK) {
      functionToCall = function () {
        setRGBLED(0, this.hub.getConnectionDetails());
      }.bind(this);
    }
    else if (data == WHITE) {
      functionToCall = function () {
        setRGBMode(MODEPREV, this.hub.getConnectionDetails());
      }.bind(this);
      turnOnFirst = true;
    }
      ...
      if (colourIndex != -1) { //is a colour
        functionToCall = function() {
          setRGBColour(lllColourLookup[colourIndex], this.hub.getConnectionDetails());
        }.bind(this);
      turnOnFirst = true;
      }
    }
    if (typeof functionToCall !== 'undefined') {
      if (turnOnFirst) {
        setRGBLED(1, this.hub.getConnectionDetails(),  functionToCall);
      }
      else {
        functionToCall();
      }
...
LimitlessLightGroup.prototype.writeRGBW = function( data ) {
  ...
  if ( data.on ) {
    ...
    self.sendRGBWGroupCommand( 'on', function() {
      ...
      var whiteMode = ( a.sat === 0 ); // if a color temp is specified, say we're white
...
LimitlessLightGroup.prototype.sendDirectBrightnessCommand = function( brightness, callback ) {
  ...
  var cmd = 0x4E;
  sendUDPCommand(cmd, parseInt(brightness * 0xff), this.hub.getConnectionDetails(), callback);
};
...
LimitlessLightGroup.prototype.sendRGBColorCommand = function( hueVal, callback ) {
  ...
  sendUDPCommand(cmd, hueVal, this.hub.getConnectionDetails(), callback);
};
...
LimitlessLightGroup.prototype.sendRGBWhiteColorCommand = function( callback ) {
  ...
  self.repeatCommand( function( callback ) {
    sendUDPCommand(cmdModePrevByte, 0, self.hub.getConnectionDetails(), callback, 50);
  }, 20, callback );
};
...
LimitlessLightGroup.prototype.sendBrightnessUpCommand = function(callback) {
  ...
  sendUDPCommand(cmdByte, valByteNA, this.hub.getConnectionDetails(), callback);
};
